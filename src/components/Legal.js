...
• If you switch on "Read codes in pictures" (it is off unless you turn it on, and needs the macOS Screen Recording permission), it also takes a picture of the window in front, reads any inkk code in it on your Mac, and deletes the picture at once. Nothing is recorded or sent apart from a code it finds.
• When you save or export a document in an app you have been writing in, it certifies the document's text (read from the file, on your Mac) and writes the code into the file's metadata: its extended attributes, and for Word documents and PDFs their document properties. Before changing a Word document or PDF it keeps a copy of the original in its own folder for seven days. You can turn this off in Settings.
• When you sign an email with its shortcut, it certifies the email you have written (read once through Accessibility, like any certificate), puts a picture of your name on the clipboard and pastes it. The picture links to the certificate and carries its code in its description and in slight variations of its ink; where only text can be pasted, the code follows your name in invisible characters. So that web mail can show it, the picture is uploaded with the certificate and served from inkk.site at an address made from the code, public like the certificate itself. The name you sign with is stored on your Mac and is written into that certificate as its author.
• It records which app you were typing in (its name and bundle identifier) and the name of the document or window, so that sessions can be grouped per piece of writing. These stay on your Mac.
• So that a piece written over several sittings keeps one record and one code, while you type it reads the text of the document you are typing in, on your Mac, every so often, and turns each sentence into a short fingerprint made with a key that exists only on your Mac. It keeps those fingerprints, never the text, and compares them the next time you type in a document to recognise a piece you have written in before. The fingerprints cannot be turned back into your words, and without the key they mean nothing even on another Mac.
...
When you certify a piece, one row is added to an append-only ledger: the code (INKK-XXXX-XXXX-XXXX), the title you gave the piece (optional), your author name and username if your account has them, the SHA-256 fingerprint of the text and a short fingerprint of each of its sentences (so a reader's copy can be checked sentence by sentence without the text being stored), its word count, the Human Signal score and tier, whether it met the verified threshold, the time it was issued, and your internal user ID. The ledger never holds the text of the piece and never holds keystroke data.

Anyone who has a code can look it up at inkk.site/certify and will see the code, title, author name and username, fingerprints, word count, score, tier and the date it was issued. Please choose your title and name with that in mind: they are the only human-readable parts of a certificate. A code cannot be edited once issued; editing a piece and certifying it again issues a new code.
